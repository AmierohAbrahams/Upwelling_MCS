...
# Exceedence function heatwaveR - This is done to determine the consecutive number of days at or above what the UI value is ment to be 
...
SACTN_upwell <- SACTN_US %>% 
  ...
  group_by(site) %>%
  nest() %>% 
  mutate(exceed = purrr::map(data, exceedance, threshold = 12.4, below = TRUE))

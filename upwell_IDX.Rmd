...
##################### Upwelling ######################
# Min duration for an upwelling event - 1day and threshold -1. Upwelling can over within a few hours, but given that we have daily data - upwelling has a minimum duration of 1 day
# Seasonal upwelling. Upwelling seasons are summer and spring
# Did the wind intensify? This is usually done by taking first third of the dataset and the last thid of the dataset and doing a t-test on this
# Strong south easterly winds lead to a stronger upwelling event
##############################################
...
# Wind speed intensifed over the years
...
wind_daily_season_test <- wind_daily_season %>% 
  group_by(season, temp_sites) %>% 
  arrange(mean_speed) %>% 
  # Add the following line with mutate to filter the data and add a text label to use with an ANOVA

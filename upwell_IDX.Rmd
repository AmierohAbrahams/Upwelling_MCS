...
UI_angle <- wind_daily %>% 
  ...
  mutate(ui.saws = mean_speed * (cos(dir_circ - coast_angle))) %>% # Calculated perpendicular angle for each of the sites. Not necessary
  drop_na %>% 
  dplyr::rename(t = date)
...
# Exceedence function heatwaveR - This is done to determine the consecutive number of days at or above what the UI value is meant to be 
...
SACTN_upwell <- UI_angle %>% 
  ...
  #        max_t = max(t)) %>% 
  nest() %>% 
  mutate(clim = purrr::map(data, ts2clm, climatologyPeriod = c("2000-01-01", "2005-12-31")),
...
SACTN_upwell_base <- SACTN_US %>% 
  ...
  mutate(clim = purrr::map(data, ts2clm, pctile = 20, climatologyPeriod = c("1995-01-01", "2004-12-31"))) %>%
  select(-data) %>% 
  unnest() %>%
...
ggplot(SP_upwelling, aes(x = t, y = temp)) +
...
# We can see in the figure above that the rug plot, which denotes when the wind driven upwelling was detected,
# occurrs much less frequently than simply using the 30th percentile threshold.
...
Increase in upwelling over the past years as a result of intensified winds over time?
Weakening of th upwelling over time, leads to a decrease in the upwelling indexs
Strong south easterly winds - Spring and summer: Upwelling is more intense in spring and summer months
The formula used suggests that if values at + upwelling occurs and if values are - then downwelling occurs. But the intensity of upwelling changes.
Compare upwelling indeces per season 


```{r}
## Adding a seasonal component to the UI_angle dataset

season_func <- function(df){
  season <- df %>%
  mutate(month = month(t, abbr = T, label = T),
         year = year(t)) %>%
...
# UI_angle_season <- season_func(df = UI_angle)
# save(UI_angle_season, file = "UI_angle_season.RData")

load("Data/UI_angle_season.RData")

# Speed of wind increased over time???
anova_func <- function(df){
  season_aov <- aov(mean_speed ~ site  * year * season, data = df)
return(season_aov)
}

wind_speed_aov <- anova_func(df = UI_angle_season)
summary(wind_speed_aov)

anova_func <- function(df){
  season_aov <- aov(ui.saws ~ site  * year * season, data = df)
return(season_aov)
}
ui.saws_aov <- anova_func(df = UI_angle_season)
summary(ui.saws_aov)
```

# Wind speed intensifed ovwer the years
Take first third (33percentile) of the data and the last third (66th percentile) of the data for every season and then do a t-test
...
# wind_daily_season <- season_func(df = wind_daily)

#save(wind_daily_season, file = "Data/wind_daily_season.RData")

load("Data/wind_daily_season.RData")

wind_daily_season_HB <- wind_daily_season %>% 
  filter(temp_sites == "Hout Bay") %>% 
  group_by(season) %>% 
  arrange(mean_speed) %>% 
  summarize(msp_1st = quantile(mean_speed, probs=0.33, na.rm=TRUE),
            mspeed_2nd = quantile(mean_speed, probs=0.66, na.rm=TRUE))

wind_daily_season_SP <- wind_daily_season %>% 
  filter(temp_sites == "Sea Point") %>% 
  group_by(season) %>% 
  arrange(mean_speed) %>% 
  summarize(msp_1st = quantile(mean_speed, probs=0.33, na.rm=TRUE),
            mspeed_2nd = quantile(mean_speed, probs=0.66, na.rm=TRUE))

wind_daily_season_SB <- wind_daily_season %>% 
  filter(temp_sites == "Saldanha Bay") %>% 
  group_by(season) %>% 
  arrange(mean_speed) %>% 
  summarize(msp_1st = quantile(mean_speed, probs=0.33, na.rm=TRUE),
            mspeed_2nd = quantile(mean_speed, probs=0.66, na.rm=TRUE))

wind_daily_season_PN <- wind_daily_season %>% 
  filter(temp_sites == "Port Nolloth") %>% 
  group_by(season) %>% 
  arrange(mean_speed) %>% 
  summarize(msp_1st = quantile(mean_speed, probs=0.33, na.rm=TRUE),
            mspeed_2nd = quantile(mean_speed, probs=0.66, na.rm=TRUE))

wind_daily_season_LB <- wind_daily_season %>% 
  filter(temp_sites == "Lamberts Bay") %>% 
  group_by(season) %>% 
  arrange(mean_speed) %>% 
  summarize(msp_1st = quantile(mean_speed, probs=0.33, na.rm=TRUE),
            mspeed_2nd = quantile(mean_speed, probs=0.66, na.rm=TRUE))
...
# T-test
t.test(wind_daily_season_HB$msp_1st, wind_daily_season_HB$mspeed_perc, paired=TRUE)

# T- test between seasons for all sites

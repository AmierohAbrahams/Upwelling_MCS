...
# The data provided by SAWS already reads wind from the direction which it is blowing
# RWS: Great find on this timeAverage function!
...
<!-- RWS: This section is no longer necessary as we decided to go with the static value of 160 -->
...
south_africa_along_wide <- coastR::transects(site_list_sub, spread = 30, alongshore = T) 
south_africa_away_wide <- coastR::transects(site_list_sub, spread = 30, alongshore = F) %>% 
...
south_africa_along <- plot_sites(south_africa_along_wide, 1, 100000)
south_africa_along
```
...
```{r}
# RWS: the cos() function works on radians, not degrees.
UI_angle <- wind_daily %>% # Making reference to the wind daily data created above
  ...
  drop_na()
# save(UI_angle, file = "Data_coast_angle/UI_angle.RData")
# save(UI_angle, file = "Data/UI_angle.RData")

# RWS: Can the following code be deleted now?
# index <- wind_UI %>%
#   group_by(site) %>%
#   select("date", "ui.saws") %>%
...
SACTN_upwell <- UI_angle %>% 
  ...
  # mutate(min_t = min(t), 
  #        max_t = max(t)) %>% 
  nest() %>% # apply the following functions to all of the variables in the dataset
  # RWS: Please stop using uneven periods for your climatology periods 12 month periods should be used.
  mutate(clim = purrr::map(data, ts2clm, climatologyPeriod = c("1992-07-09", "2017-12-21")), # creating a column of climatologies. Column will be named clim
         ...
         # purr::map - applies a function to each element of a vector
         exceed = purrr::map(clim, exceedance, minDuration = 1, threshold = 1)) %>%  # Upwelling cannot be described as an event. Upwelling can last for a few hours. Given that we have daily data, upwelling events minimum duration here will be 1day
  # Detect consecutive days in exceedance of a given threshold.
  # mutate() %>% 
  select(-data, -clim) %>% 
  unnest(cols = exceed) %>%
  filter(row_number() %% 2 == 1) %>%
  unnest(cols = exceed) %>% # creates a column for each variables
...
# RWS: If with the above code your primary objective is to determine when ui.saws is above 1, that may be done with the following code
SACTN_upwell <- UI_angle %>% 
...
# The first is to calculate rolling climatology and to check the upwelling phenology against that
...
SACTN_upwell_base <- SACTN_US %>% 
  ...
  # The choice of the 30th percentile threshold also needs to be justified and sensitivity tested
  mutate(clim = purrr::map(data, ts2clm, pctile = 25, climatologyPeriod = c("1992-07-09", "2005-10-19"))) %>%
  select(-data) %>% 
  unnest(cols = clim) %>%

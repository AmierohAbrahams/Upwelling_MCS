...
SACTN_upwell <- UI_angle %>% 
  ...
  mutate(clim = purrr::map(data, ts2clm, climatologyPeriod = c("2000-01-01", "2005-12-31")),
         ...
         exceed = purrr::map(clim, exceedance, minDuration = 2, threshold = 1)) %>% 
  # mutate() %>% 
  select(-data, -clim) %>% 
  unnest() %>%
...
detect_event_custom <- function(df){
  res <- detect_event(df, threshClim2 = df$exceedance, minDuration = 2, coldSpells = T)
  return(res)
}
...
SACTN_upwell_metrics <- SACTN_upwell_base %>% 
  unnest() %>%

...
# https://cran.r-project.org/web/packages/rWind/rWind.pdfsave(, file = "Data_coastal_angle/wind_daily.RData")
# https://www.kaggle.com/srlightfoote/example-wind-analysis-in-r
# The data provided by SAWS already reads wind from the direction which it is blwing
  
#AA: NA values appear when applying the circular function...
load(Data_coast_angle/wind_daily_tested.RData)
...
# library(openair)
# wind_daily_speed <- timeAverage(wind_data, avg.time = "day")
# # save(wind_daily, file = "Data/wind_daily.RData") 
# #Convert Wind Speed and Wind Direction to u/v Wind Components 
# # Then get the daily average and then convert it back?
# # https://rdrr.io/github/retostauffer/Rfoehnix/man/ddff2uv.html
# # Use with zoo
# 
# library("zoo")
# set.seed(100)
...
UI_angle <- wind_daily %>%  # Making reference to the wind daily data created above
  ...
  mutate(ui.saws = mean_speed * (cos(dir_circ - 160))) %>% # This is the rientation of the coastal angle for the west coast
  dplyr::rename(t = date) %>% 
...
SACTN_upwell <- UI_angle %>% 
  ...
  mutate(clim = purrr::map(data, ts2clm, climatologyPeriod = c("1992-07-09", "2017-12-21")), # creating a column of climaatologies. Column will be named clim

...
# Ned to include distance and product
...
Comparing the relaionship of variables within the same sample
  ...
  -Correlation of the number of frequencies at different distances at each site
  
```{r}
cor.test(x = Combined_frequency$distance, Combined_frequency$number_of_signals,
...
distance_corr <- Combined_frequency %>% 
  group_by(site) %>% 
  slope_calc()
...
load("Data/G1SST_upwell_base.RData")
    ...
    #Why are there no signals in 2011 in the SACTN data?
    # Why are some signals detected in the SACTN data? Is this because of the intensity of the signals?


# Reasons why upwelling might be detected at 10km rather than 50km
    # Dome in the isotherm blocking upwelling to reach the surface waters but does occur deeper down
# (Figures folder - ig 2 and figure 10)
```

...
score_model <- lm(intensity_mean ~ no_of_signals, data = Regression)
final_regression <- get_regression_table(score_model)
  ...
  linear_regression <- Regression %>% 
    ...
    na.omit() %>% 
    nest() %>%
    mutate(model = purrr::map(data, ~lm(variable ~ value, data = .))) %>%

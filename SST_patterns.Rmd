...
# nc_data <- nc_open('/home/amieroh/Documents/Data/Datasets/MUR/daily/20020601-JPL-L4UHfnd-GLOB-v01-fv04-MUR.nc')
# print(nc_data)

ncDir <- "/home/amieroh/Documents/Data/Datasets/MUR/daily"
csvDir <- "/home/amieroh/Documents"
...
```{r}
ncDir <- "/home/amieroh/Documents/Data/Datasets/CMC/CMC_global"
csvDir <- "/home/amieroh/Documents"
...
read_nc <- function(ncFile = ncFile, region = region, csvDir = csvDir) {
  coords <- bbox[,region]
    nc <- nc_open(ncFile)
    pathLen <- nchar(paste0(ncDir, "/", region)) + 1
    fNameStem <-
      substr(ncFile, pathLen + 10, pathLen + 38)
...
# the list of files
ncList <- list.files(path = ncDir, pattern = "*.nc", full.names = TRUE, include.dirs = TRUE)
strtDate <- str_sub(ncList[1], start = 1, end = 8)
endDate <- str_sub(ncList[length(ncList)], start = 1, end = 8)

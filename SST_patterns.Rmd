...
# load("~/Documents/Upwelling_MCS/Data/OISST_sites.RData") # OISST
# load("~/Documents/Upwelling_MCS/Data/CMC_SST.RData") # CMC temperature
```
...
## Creating a time series graph with all of the products but showing the temperature drop but with no CMC
...
CMC_temp <- CMC %>% 
  ...
  mutate(product = "CMC") %>% 
  dplyr::select(date,site,lat,lon,temp,product)#%>% 
 # select(date,site,product,lat,lon,temp)
  
OISST_temp <- OISST_sites %>% 
  filter(site == "Lamberts Bay",
         year(date)== 2014)%>% 
  mutate(product = "OISST")%>% 
  dplyr::select(date,site,lat,lon,temp,product)
...
G1SST_temp <- Final_G1SST %>% 
  filter(site == "Lamberts Bay",
         year(date) == 2014) %>% 
  mutate(product = "G1SST")%>% 
  dplyr::select(date,site,lat,lon,temp,product)
  
combined_SST <- rbind(CMC_temp,MUR_temp,G1SST_temp,OISST_temp)

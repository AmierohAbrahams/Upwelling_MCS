...
Plot showing upwelling at the different sites for each of the different distances

```{r}

scatter_func <- function(df){
  CMC_filtered <- df %>%
filter(year(date_start) %in% seq(2010, 2016, 1))
}

CMC_filtered <- scatter_func(df = CMC_upwell_base)
OISST_filtered <- scatter_func(df = OISST_upwell_base)
SACTN_filtered <- scatter_func(df = SACTN_upwell_base)
...
(CMC_plot <- plot_scat_func(df = CMC_filtered))
(OISST_plot <- plot_scat_func(df = OISST_filtered))
...
(CMC_plot <- plot_loli_func(df = CMC_filtered))
(OISST_plot <- plot_loli_func(df = OISST_filtered))

...
site_pixels$lat<-round(site_pixels$lat, 1) #CMC is to 1 decimal place and OISST is to 2
site_pixels$lon<-round(site_pixels$lon, 1)
...
match_func <- function(df){
  ...
  match_index <- knnx.index(data = as.matrix(round(site_pixels[,2:3]),1),
...
pixel_match$lat<-round(pixel_match$lat, 1) #CMC is to 1 decimal place and OISST is to 2
pixel_match$lon<-round(pixel_match$lon, 1)

# You may then use the 'pixel_match' object to filter out the desired pixels from the full satellite products
# one time series for each product for each pixel at each distance for each site that you may then
# dataset_filt <- function(df){
#   filtered_products <- df %>% 
#   unite(col = combi, lon, lat, sep = "/", remove = F) #%>% 
#   #filter(combi %in% pixel_match$combi)
# }

# OISST_fil <- dataset_filt(df = OISST)
# CMC_fil <- dataset_filt(df = CMC)
# 
# 
# pixel_match$lat
# pixel_match$lon

# Flterning out the data from the desired pixels. Here the distances from the coastline is not shown?
filterted_product <- function(df){
  extracted <- pixel_match %>%
  right_join(df, by = c("lon", "lat")) %>% 
  select(date,temp,lat,lon) 
}

OISST_fil <- filterted_product(df = OISST) # Need to add a site column
CMC_fil <- filterted_product(df = CMC) # Need to add a site column

# OISST_sites <- OISST_fil %>% 
#   left_join(site_list_sub[,c(2,3)], by = c("lat","lon"))
...
upwelling <- UI_angle %>% 
  ...
  mutate(clim = purrr::map(data, ts2clm, climatologyPeriod = c("1997-01-01", "2015-12-31")), # creating a column of climaatologies. Column will be named clim
...
upwell_base <- df %>% 
  ...
  mutate(clim = purrr::map(data, ts2clm, pctile = 25, climatologyPeriod = c("1989-01-01", "2017-12-31"))) %>%
  select(-data) %>% 
  unnest() %>%
...
OISST_upwell_base <- upwelling_detect_event(df = OISST_fil)

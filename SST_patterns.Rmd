...
#write.csv(SACTN_upwell_base ,'SACTN_upwell_base .csv') # Copied this and then added in a product
rm(SACTN_upwell_base)
SACTN_mean_intensity <- SACTN_upwell_base_ %>% 
  select(site, product, intensity_mean)

Complete_mean_intensity <- rbind(CMC_binding,OISST_binding, G1SST_binding, MUR_binding) %>% 
  select(-heading, - distance, - lat, -lon)
# write.csv(Complete_mean_intensity ,'Complete_mean_intensity .csv') # Copied this and then removedsome columnss

final_intensity <- rbind(SACTN_mean_intensity, Complete_mean_intensity_)

ggplot(data = final_intensity, aes(x = product, y = intensity_mean)) +
  geom_point() +

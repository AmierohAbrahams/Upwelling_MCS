...
site_map <- ggplot(data = south_africa_coast, aes(x = lon, y = lat)) + 
  ...
  geom_point(data = site_list_sub, aes(x = lon, y = lat), alpha = 0.8, size = 5) +
  geom_point(data = site_pixels, aes(x = lon, y = lat), colour = "black", shape = 0, alpha = 0.8, size = 2.1) +
  geom_label_repel(data =site_list_sub_map, aes(x = lon, y = lat, label = site), 
                     size = 4, box.padding = 2, nudge_x = 0.7, nudge_y = 0.2, 
                     segment.alpha = 0.4, force = 0.1) +
  coord_equal(xlim = c(13, 23), ylim = c(-36,-26), expand = 0) +
  ...
  #            size = 4, angle = 0, colour = "black") +
    annotate("text", label = "ATLANTIC\nOCEAN", x = 15.00, y = -33.0,
             size =6, angle = 0, colour = "black") +
    annotate("text", label = "Benguela", x = 14.5, y = -30,
  ...
  theme_Publication() # R publication theme

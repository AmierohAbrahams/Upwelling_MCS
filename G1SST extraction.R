...
rm(G1SST)
ncDir <- "/home/amieroh/Documents/Data/Datasets/G1SST/daily/Extract 47.2"
...
G1SST <- read_csv("/media/amieroh/Seagate EXpansion/Extract/JPL_OUROCEAN-L4UHfnd-GLOB-v01-fv01_0-G1SST_subset-20120524-20120601.csv")
names(G1SST)<-c("lon","lat", "temp", "date")
G1SST <- G1SST %>%
  mutate(temp = temp - 273.15)

save(G1SST, file = "/media/amieroh/Seagate EXpansion/G1SST/G1SST_47.2.RData")
#####################333
rm(G1SST)
rm(Final)
rm(G1SST_1)
...
rm(sat_data)
rm(sat_pixels)
load("/media/amieroh/Seagate EXpansion/G1SST/G1SST_40.2.RData")
G1SST_1 <- G1SST %>% 
  na.omit()
load("/media/amieroh/Seagate EXpansion/G1SST/G1SST_47.2.RData")
G1SST_2 <- G1SST %>% 
  na.omit()
